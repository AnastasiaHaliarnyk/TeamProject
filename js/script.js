(function (global) {
    ...
    function buildCatalogItemsViewHtml(categoryCatalogItems, catalogItemsTitleHtml, catalogItemPreviewHtml) {
        ...
        for (let i = 0; i < catalogItems.length; i++) {
            ...
            html = insertItemPrice(html, "elavation", catalogItems[i].elavation);
            html = insertProperty(html, "categoryShortName", categoryShortName);
            html = insertItemArea(html, "distance", catalogItems[i].distance);
    ...
    let insertItemPrice = function (html, pricePropName, priceValue) {
        ...
        priceValue = priceValue.toFixed(2) + " f";
    ...
    let insertItemArea = function (html, areaPropName, areaValue) {
        ...
        areaValue = areaValue.toFixed(2) + " mi";
    ...
    function buildItemViewHtml(categoryItems, itemToDisplayId, itemHtml) {
        ...
        for (let i = 0; i < catalogItems.length; i++) {
            if (catalogItems[i].id == itemToDisplayId) {
                ...
                finalHtml = insertItemArea(finalHtml, "distance", catalogItems[i].distance);
                finalHtml = insertItemPrice(finalHtml, "elavation", catalogItems[i].elavation);
                return finalHtml;
            }
        }
    }
    ...
    ns.getCalculate = function() {
        let name = document.getElementById("floatingInputM").value;
        let hikes = ["Ivano-Frankivsk", "Kyiv", "Lviv", "Ternopil", "Zakarpatya"];
        let randomHike = hikes[Math.floor(Math.random() * hikes.length)];

        if (name.trim() !== "") {
            alert("Hello " + name + "! Here's the hiking oblast for you: " + randomHike);
        } else {
